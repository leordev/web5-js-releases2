...
export class Secp256r1 {
  ...
  public static async getPublicKey({ key }: GetPublicKeyParams): Promise<Jwk> {
    ...
    if (!(isEcPrivateJwk(key) && key.crv === "P-256")) {
      throw new Error(
        `Secp256r1: The provided key is not a 'P-256' private JWK.`
      );
    }
  ...
  public static async privateKeyToBytes({
    ...
    if (!isEcPrivateJwk(privateKey)) {
      throw new Error(
        `Secp256r1: The provided key is not a valid EC private key.`
      );
    }
  ...
  public static async publicKeyToBytes({
    ...
    if (!(isEcPublicJwk(publicKey) && publicKey.y)) {
      throw new Error(
        `Secp256r1: The provided key is not a valid EC public key.`
      );
    }
  ...
  public static async sharedSecret({
    ...
    if (
      ...
      throw new Error(
        `Secp256r1: ECDH shared secret cannot be computed from a single key pair's public and private keys.`
      );
    }

    // Convert the provided private and public keys to bytes.
    const privateKeyABytes = await Secp256r1.privateKeyToBytes({
      privateKey: privateKeyA,
    });
    const publicKeyBBytes = await Secp256r1.publicKeyToBytes({
      publicKey: publicKeyB,
    });

    // Compute the compact representation shared secret between the public and private keys.
    const sharedSecret = secp256r1.getSharedSecret(
      privateKeyABytes,
      publicKeyBBytes,
      true
    );
  ...
  public static async verify({
    ...
    const isValid = secp256r1.verify(signature, digest, publicKeyBytes, {
      lowS: false,
    });
