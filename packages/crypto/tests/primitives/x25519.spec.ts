...
describe("X25519", () => {
  ...
  describe("computePublicKey()", () => {
    ...
    it("computes and adds a kid property, if missing", async () => {
      ...
      const publicKey = await X25519.computePublicKey({
  ...
  describe("getPublicKey()", () => {
    ...
    it("computes and adds a kid property, if missing", async () => {
      const { kid, ...privateKeyWithoutKid } = privateKey;
      const publicKey = await X25519.getPublicKey({
        key: privateKeyWithoutKid,
      });
    ...
    it("returns the same output as computePublicKey()", async () => {
      ...
      expect(publicKey).to.deep.equal(
        await X25519.computePublicKey({ key: privateKey })
      );
    });
    ...
    it("throws an error when provided an X25519 public key", async () => {
      await expect(
        ...
        Error,
        "key is not an X25519 private JWK"
      );
    });
  ...
  describe("privateKeyToBytes()", () => {
    it("returns a private key as a byte array", async () => {
      ...
      const expectedOutput = Convert.hex(
        "8f14925ff68ce3d9ba13831a49dfa1722cc8337deb5f32e5b6e7aff68070d55f"
      ).toUint8Array();
    ...
    it("throws an error when provided an X25519 public key", async () => {
      ...
      await expect(
        ...
        Error,
        "provided key is not a valid OKP private key"
      );
    });
  ...
  describe("publicKeyToBytes()", () => {
    it("returns a public key in JWK format", async () => {
      ...
      const expectedOutput = Convert.hex(
        "536917d857244c0a1302330151a7703a97ed75793e2b1f2964c7b21b7419b32f"
      ).toUint8Array();
    ...
    it("throws an error when provided an X25519 private key", async () => {
      ...
      await expect(
        ...
        Error,
        "provided key is not a valid OKP public key"
      );
    });
  ...
  describe("sharedSecret()", () => {
    ...
    before(async () => {
      ...
      otherPartyPublicKey = await X25519.computePublicKey({
