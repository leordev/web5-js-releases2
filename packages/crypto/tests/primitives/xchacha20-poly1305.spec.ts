...
import {
  POLY1305_TAG_LENGTH,
  XChaCha20Poly1305,
} from "../../src/primitives/xchacha20-poly1305.js";
...
describe("XChaCha20Poly1305", () => {
  describe("bytesToPrivateKey()", () => {
    it("returns a private key in JWK format", async () => {
      const privateKeyBytes = Convert.hex(
        "ffbd52af5980bd3870cdc3f3634980ae9d15b33440f63f79799eb8ca2329117f"
      ).toUint8Array();
      const privateKey = await XChaCha20Poly1305.bytesToPrivateKey({
        privateKeyBytes,
      });
    ...
    it("returns the expected JWK given byte array input", async () => {
      const privateKeyBytes = Convert.hex(
        "2fbd52af5980bd3870cdc3f3634980ae9d15b33440f63f79799eb8ca2329117f"
      ).toUint8Array();
  ...
  describe("decrypt()", () => {
    it("returns Uint8Array plaintext with length matching input", async () => {
      ...
      const tag = Convert.hex(
        "09701fb9f36ab77a0f136ca539229a34"
      ).toUint8Array();
      const plaintext = await XChaCha20Poly1305.decrypt({
        data: new Uint8Array([...ciphertext, ...tag]),
        key: await XChaCha20Poly1305.bytesToPrivateKey({
    ...
    it("passes test vectors", async () => {
      ...
      const ciphertext = Convert.hex(
        "80246ca517c0fb5860c19090a7e7a2b030dde4882520102cbc64fad937916596ca9d"
      ).toUint8Array();
      const tag = Convert.hex(
        "9e10a121d990e6a290f6b534516aa32f"
      ).toUint8Array();
      const nonce = Convert.hex(
      ...
      const output = Convert.string(
        `Are You There Bob? It's Me, Alice.`
      ).toUint8Array();
  ...
  describe("privateKeyToBytes()", () => {
    it("returns a private key as a byte array", async () => {
      const privateKey = await XChaCha20Poly1305.generateKey();
      const privateKeyBytes = await XChaCha20Poly1305.privateKeyToBytes({
        privateKey,
      });
    ...
    it("throws an error when provided an asymmetric public key", async () => {
      ...
      await expect(
        ...
        Error,
        "provided key is not a valid oct private key"
      );
    });
  });
});
