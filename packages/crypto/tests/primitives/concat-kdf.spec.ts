...
describe("ConcatKdf", () => {
  describe("deriveKey()", () => {
    ...
    it("throws an error if suppPubInfo is not a Number", async () => {
      await expect(
        ...
        TypeError,
        "Fixed length input must be a number"
      );
    });
  });
  ...
  describe("computeFixedInfo()", () => {
    it("returns concatenated and formatted Uint8Array", () => {
      ...
      const output =
        "AAAAB0ExMjhHQ00AAAAFQWxpY2UAAAADQm9iAAAAgAAAACtnSTBHQUlMQmR1N1Q1M2FrckZtTXlHY3NGM241ZE83TW13TkJIS1c1U1Yw";
