...
describe("AesGcm", () => {
  ...
  describe("decrypt", () => {
    it("returns ciphertext as a Uint8Array", async () => {
      // Setup.
      const privateKeyBytes = Convert.hex(
        "de4ea2de269297bfbf4be347dcdd4631"
      ).toUint8Array();
      const privateKey = await AesGcm.bytesToPrivateKey({ privateKeyBytes });
      const ciphertext = Convert.hex(
        "f27e81aa63c315a5cd03e2abcbc62a5665"
      ).toUint8Array();
    ...
    for (const vector of AesGcmDecryptTestVector.vectors) {
      it(vector.description, async () => {
        ...
        const plaintext = await AesGcm.decrypt({
          ...
          additionalData: Convert.hex(
            vector.input.additionalData
          ).toUint8Array(),
          tagLength: vector.input
            .tagLength as (typeof AES_GCM_TAG_LENGTHS)[number],
        });
    ...
    it("throws an error if the initialization vector is not 96 bits in length", async () => {
      ...
      for (const ivLength of [32, 64, 100, 256]) {
        ...
        try {
          ...
          expect.fail(
            "expected an error to be thrown due to unsupported initialization vector length"
          );
        } catch (error: any) {
          // Validate the result.
    ...
    it("throws an error if the tag length is invalid", async () => {
      ...
      for (const tagLength of [32, 64, 100, 256]) {
        try {
          ...
          expect.fail(
            "expected an error to be thrown due to invalid tag length"
          );
        } catch (error: any) {
          // Validate the result.
  ...
  describe("generateKey()", () => {
    ...
    it("supports key lengths of 128 and 256 bits in all supported runtimes", async () => {
      ...
      // 128 bits
      privateKey = (await AesGcm.generateKey({
        length: 128,
    ...
    it("throws an error if the key length is invalid", async () => {
      for (const length of [32, 64, 100, 512]) {
        try {
          ...
          expect.fail(
            "expected an error to be thrown due to invalid key length"
          );
        } catch (error: any) {
          // Validate the result.
