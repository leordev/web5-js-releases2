...
describe("AesCtr", () => {
  ...
  describe("decrypt", () => {
    ...
    for (const vector of AesCtrDecryptTestVector.vectors) {
      it(vector.description, async function () {
        ...
        expect(ciphertext).to.deep.equal(
    ...
    it("throws an error if the initial counter block is not 128 bits in length", async () => {
      ...
      for (const counterLength of [64, 192, 256]) {
        ...
        try {
          ...
          expect.fail(
            "expected an error to be thrown due to invalid counter length"
          );
        } catch (error: any) {
          // Validate the result.
    ...
    it("throws an error if the counter length is not in the range 1 to 128 bits", async () => {
      ...
      for (const length of [0, 129, 256]) {
        try {
          ...
          expect.fail(
            "expected an error to be thrown due to invalid length property"
          );
        } catch (error: any) {
          // Validate the result.
  ...
  describe("encrypt", () => {
    it("accepts plaintext input as Uint8Array", async () => {
      ...
      // Uint8Array
      ciphertext = await AesCtr.encrypt({
        counter: new Uint8Array(16),
        data,
        key: privateKey,
        length: 128,
  ...
  describe("generateKey()", () => {
    ...
    it("supports key lengths of 128 and 256 bits in all supported runtimes", async () => {
      ...
      // 128 bits
      privateKey = (await AesCtr.generateKey({
        length: 128,
