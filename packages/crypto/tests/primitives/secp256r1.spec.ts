...
describe("Secp256r1", () => {
  ...
  describe("adjustSignatureToLowS()", () => {
    it("returns a 64-byte signature of type Uint8Array", async () => {
      ...
      const adjustedSignature = await Secp256r1.adjustSignatureToLowS({
        signature,
      });
    ...
    it("returns signatures that can be verified regardless of low- or high-S form", async () => {
      ...
      let adjustedSignature = await Secp256r1.adjustSignatureToLowS({
        signature: signatureLowS,
      });
      let isValid = await Secp256r1.verify({
        key: publicKey,
      ...
      adjustedSignature = await Secp256r1.adjustSignatureToLowS({
        signature: signatureHighS,
      });
      isValid = await Secp256r1.verify({
        key: publicKey,
        signature: adjustedSignature,
        data,
      });
  ...
  describe("bytesToPrivateKey()", () => {
    it("returns a private key in JWK format", async () => {
      const privateKeyBytes = Convert.hex(
        "08169cf81812f2e288a1131de246ebdf29b020c7625a98d098296a30a876d35a"
      ).toUint8Array();
    ...
    for (const vector of secp256r1BytesToPrivateKey.vectors) {
      it(vector.description, async () => {
        const privateKey = await Secp256r1.bytesToPrivateKey({
          privateKeyBytes: Convert.hex(
            vector.input.privateKeyBytes
          ).toUint8Array(),
        });
  ...
  describe("bytesToPublicKey()", () => {
    it("returns a public key in JWK format", async () => {
      const publicKeyBytes = Convert.hex(
        "048b542fa180e78bc981e6671374a64413e0323b439d06870dc49cb56e97775d96a0e469310d10a8ff2cb253a08d46fd845ae330e3ac4e41d0d0a85fbeb8e15795"
      ).toUint8Array();
      const publicKey = await Secp256r1.bytesToPublicKey({ publicKeyBytes });
    ...
    for (const vector of secp256r1BytesToPublicKey.vectors) {
      it(vector.description, async () => {
        const publicKey = await Secp256r1.bytesToPublicKey({
          publicKeyBytes: Convert.hex(
            vector.input.publicKeyBytes
          ).toUint8Array(),
        });
  ...
  describe("compressPublicKey()", () => {
    it("converts an uncompressed public key to compressed format", async () => {
      const compressedPublicKeyBytes = Convert.hex(
        "02d7251f4572325f4b1a9642600427adfe11ea3bd4dfe1cd7f4932612129e18784"
      ).toUint8Array();
      const uncompressedPublicKeyBytes = Convert.hex(
        "04d7251f4572325f4b1a9642600427adfe11ea3bd4dfe1cd7f4932612129e187844247b3c6302e7ecd611dbb666380e1117b198f37a9d183de422947f6b6183098"
      ).toUint8Array();
    ...
    it("throws an error for an invalid uncompressed public key", async () => {
      ...
      const invalidPublicKey = Convert.hex(
      ...
      try {
        ...
        expect((error as Error).message).to.include(
          "Point of length 65 was invalid"
        );
      }
    });
  });
  ...
  describe("computePublicKey()", () => {
    ...
    it("computes and adds a kid property, if missing", async () => {
      ...
      const publicKey = await Secp256r1.computePublicKey({
        key: privateKeyWithoutKid,
      });
  ...
  describe("convertDerToCompactSignature()", () => {
    it("returns compact R+S format signature as a Uint8Array", async () => {
      const derSignature = Convert.hex(
        "3045022100b292a619339f6e567a305c951c0dcbcc42d16e47f219f9e98e76e09d8770b34a02200177e60492c5a8242f76f07bfe3661bde59ec2a17ce5bd2dab2abebdf89a62e2"
      ).toUint8Array();

      const compactSignature = await Secp256r1.convertDerToCompactSignature({
        derSignature,
      });
    ...
    it("converted ASN.1 DER encoded ECDSA signature matches the expected compact R+S signature", async () => {
      ...
      const expectedCompactSignature = Convert.hex(
    ...
    it("passes Wycheproof test vector", async () => {
      ...
      const derSignature = Convert.hex(
        "304402202ba3a8be6b94d5ec80a6d9d1190a436effe50d85a1eee859b8cc6af9bd5c2e1802204cd60b855d442f5b3c7b11eb6c4e0ae7525fe710fab9aa7c77a67f79e6fadd76"
      ).toUint8Array();
    ...
    it("throws an error for an invalid ASN.1 DER encoded ECDSA signature due to incorrect length", async () => {
      ...
      const invalidDerSignature = Convert.hex(
        "304602202ba3a8be6b94d5ec80a6d9d1190a436effe50d85a1eee859b8cc6af9bd5c2e18022100b329f479a2bbd0a5c384ee1493b1f5186a87139cac5df4087c134b49156847db"
      ).toUint8Array();

      try {
        await Secp256r1.convertDerToCompactSignature({
          derSignature: invalidDerSignature,
        });
        expect.fail("Expected method to throw an error.");
      } catch (error) {
        expect(error).to.be.instanceOf(Error);
    ...
    it("throws an error for an invalid ASN.1 DER encoded ECDSA signature due to appending zeros to sequence", async () => {
      ...
      const invalidDerSignature = Convert.hex(
        "304702202ba3a8be6b94d5ec80a6d9d1190a436effe50d85a1eee859b8cc6af9bd5c2e18022100b329f479a2bbd0a5c384ee1493b1f5186a87139cac5df4087c134b49156847db0000"
      ).toUint8Array();
  ...
  describe("decompressPublicKey()", () => {
    ...
    it("throws an error for an invalid compressed public key", async () => {
      ...
      try {
        ...
        expect(error).to.be.instanceOf(Error);
        expect((error as Error).message).to.include(
          "Point of length 33 was invalid"
        );
      }
    });
  });
  ...
  describe("getCurvePoint()", () => {
    for (const vector of secp256r1GetCurvePoints.vectors) {
      it(vector.description, async () => {
        ...
        expect(points.x).to.deep.equal(
          Convert.hex(vector.output.x).toUint8Array()
        );
        expect(points.y).to.deep.equal(
          Convert.hex(vector.output.y).toUint8Array()
        );
      });
    }
    ...
    it("throws error with invalid input key length", async () => {
      await expect(
        ...
        Error,
        "Point of length 16 was invalid. Expected 33 compressed bytes or 65 uncompressed bytes"
      );
    });
  });
  ...
  describe("getPublicKey()", () => {
    ...
    it("computes and adds a kid property, if missing", async () => {
      const { kid, ...privateKeyWithoutKid } = privateKey;
      const publicKey = await Secp256r1.getPublicKey({
        key: privateKeyWithoutKid,
      });
    ...
    it("returns the same output as computePublicKey()", async () => {
      ...
      expect(publicKey).to.deep.equal(
        await Secp256r1.computePublicKey({ key: privateKey })
      );
    });
    ...
    it("throws an error when provided a secp256r1 public key", async () => {
      ...
      await expect(
        ...
        Error,
        `key is not a 'P-256' private JWK`
      );
    });
  ...
  describe("privateKeyToBytes()", () => {
    it("returns a private key as a byte array", async () => {
      ...
      const expectedOutput = Convert.hex(
        "c6a42b4e42535f61866c25bd57b4a9f082e5ab33659db545d813383a40ea637a"
      ).toUint8Array();
    ...
    it("throws an error when provided a secp256r1 public key", async () => {
      ...
      await expect(
        ...
        Error,
        "provided key is not a valid EC private key"
      );
    });
    ...
    for (const vector of secp256r1PrivateKeyToBytes.vectors) {
      it(vector.description, async () => {
        ...
        expect(privateKeyBytes).to.deep.equal(
  ...
  describe("publicKeyToBytes()", () => {
    it("returns a public key in JWK format", async () => {
      ...
      const expectedOutput = Convert.hex(
        "04c91b7390141d49f63067b107e9df5430df8f57eaf8659cd7177846cbc0fdcb2e6d051b2197aa50850a57e379dbae630fbfe5dbec726e3a45b2bde437819d03f9"
      ).toUint8Array();
    ...
    it("throws an error when provided an Ed25519 private key", async () => {
      ...
      await expect(
        ...
        Error,
        "provided key is not a valid EC public key"
      );
    });
    ...
    for (const vector of secp256r1PublicKeyToBytes.vectors) {
      it(vector.description, async () => {
        ...
        expect(publicKeyBytes).to.deep.equal(
          Convert.hex(vector.output).toUint8Array()
        );
      });
    }
  ...
  describe("sharedSecret()", () => {
    ...
    beforeEach(async () => {
      ...
      otherPartyPublicKey = await Secp256r1.computePublicKey({
        key: otherPartyPrivateKey,
      });
    });
    ...
    it("throws an error if the public/private keys from the same key pair are specified", async () => {
      await expect(
        ...
        Error,
        "shared secret cannot be computed from a single key pair"
      );
    });
  });
