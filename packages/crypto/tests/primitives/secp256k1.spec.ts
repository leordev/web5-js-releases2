...
describe("Secp256k1", () => {
  ...
  describe("adjustSignatureToLowS()", () => {
    it("returns a 64-byte signature of type Uint8Array", async () => {
      ...
      const adjustedSignature = await Secp256k1.adjustSignatureToLowS({
        signature,
      });
    ...
    it("returns the low-S form given a high-S signature", async () => {
      const signatureHighS = Convert.hex(
        "351757c538d0a13fa9473dabc259be82dba1bd8f44dcba71a7f222655429b470f9f78c954682f4ce451e5f3d353b4c9fcfbb7d702fe9e28bdfe21be648fc618d"
      ).toUint8Array();
    ...
    it("returns the signature unmodified if already in low-S form", async () => {
      const signatureLowS = Convert.hex(
        "351757c538d0a13fa9473dabc259be82dba1bd8f44dcba71a7f222655429b4700608736ab97d0b31bae1a0c2cac4b35eeaf35f767f5ebdafdff042a68739dfb4"
      ).toUint8Array();
    ...
    it("returns signatures that can be verified regardless of low- or high-S form", async () => {
      ...
      let adjustedSignature = await Secp256k1.adjustSignatureToLowS({
        signature: signatureLowS,
      });
      let isValid = await Secp256k1.verify({
        key: publicKey,
      ...
      adjustedSignature = await Secp256k1.adjustSignatureToLowS({
        signature: signatureHighS,
      });
      isValid = await Secp256k1.verify({
        key: publicKey,
        signature: adjustedSignature,
        data,
      });
  ...
  describe("computePublicKey()", () => {
    ...
    it("computes and adds a kid property, if missing", async () => {
      ...
      const publicKey = await Secp256k1.computePublicKey({
  ...
  describe("convertDerToCompactSignature()", () => {
    it("returns compact R+S format signature as a Uint8Array", async () => {
      const derSignature = Convert.hex(
        "304402203d2f8c3d0f3f7b8b0a9f4d2e8a0f2d4d7a4d2e8a0f2d4d7a4d2e8a0f2d4d7a4d02203d2f8c3d0f3f7b8b0a9f4d2e8a0f2d4d7a4d2e8a0f2d4d7a4d2e8a0f2d4d7a4d"
      ).toUint8Array();

      const compactSignature = await Secp256k1.convertDerToCompactSignature({
        derSignature,
      });
    ...
    it("converted ASN.1 DER encoded ECDSA signature matches the expected compact R+S signature", async () => {
      const derSignature = Convert.hex(
        "3046022100bd856f326c9d52c6ea6b0711831fe706ad4df6f1c2499de3aa2950d27fe89590022100be32e04c6d0d6fe1628b84eacff5bb871cea4138199521b37234da79b63586f8"
      ).toUint8Array();
      const expectedCompactSignature = Convert.hex(
        "bd856f326c9d52c6ea6b0711831fe706ad4df6f1c2499de3aa2950d27fe89590be32e04c6d0d6fe1628b84eacff5bb871cea4138199521b37234da79b63586f8"
      ).toUint8Array();
    ...
    it("throws an error for an invalid ASN.1 DER encoded ECDSA signature due to incorrect length", async () => {
      ...
      const invalidDerSignature = Convert.hex(
        "3046022100813ef79ccefa9a56f7ba805f0e478584fe5f0dd5f567bc09b5123ccbc983236502206ff18a52dcc0336f7af62400a6dd9b810732baf1ff758000d6f613a556eb31ba"
      ).toUint8Array();

      try {
        await Secp256k1.convertDerToCompactSignature({
    ...
    it("throws an error for an invalid ASN.1 DER encoded ECDSA signature due to appending zeros to sequence", async () => {
      ...
      const invalidDerSignature = Convert.hex(
        "3047022100813ef79ccefa9a56f7ba805f0e478584fe5f0dd5f567bc09b5123ccbc983236502206ff18a52dcc0336f7af62400a6dd9b810732baf1ff758000d6f613a556eb31ba0000"
      ).toUint8Array();
  ...
  describe("getPublicKey()", () => {
    ...
    it("computes and adds a kid property, if missing", async () => {
      const { kid, ...privateKeyWithoutKid } = privateKey;
      const publicKey = await Secp256k1.getPublicKey({
        key: privateKeyWithoutKid,
      });
    ...
    it("returns the same output as computePublicKey()", async () => {
      ...
      expect(publicKey).to.deep.equal(
        await Secp256k1.computePublicKey({ key: privateKey })
      );
    });
    ...
    it("throws an error when provided a secp256k1 public key", async () => {
      await expect(
        ...
        Error,
        "key is not a secp256k1 private JWK"
      );
    });
  ...
  describe("sharedSecret()", () => {
    ...
    beforeEach(async () => {
      ...
      otherPartyPublicKey = await Secp256k1.computePublicKey({
    ...
    describe("Web5TestVectorsCryptoEs256k", () => {
      it("sign", async () => {
        for (const vector of CryptoEs256kSignTestVector.vectors) {
          ...
          expect(errorOccurred).to.equal(
            vector.errors,
            `Expected '${vector.description}' to${
              vector.errors ? " " : " not "
            }throw an error`
          );
        }
