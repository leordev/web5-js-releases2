...
import { Convert, MemoryStore } from "@leordev-web5/common";
...
describe("LocalKeyManager", () => {
  ...
  describe("sign()", () => {
    it("generates signatures as Uint8Array", async () => {
      // Setup.
      const privateKeyUri = await keyManager.generateKey({
        algorithm: "secp256k1",
      });
      const data = new Uint8Array([0, 1, 2, 3, 4]);
  ...
  describe("verify()", () => {
    ...
    it("throws an error when public key algorithm and curve are unsupported", async () => {
      ...
      try {
        ...
        expect(error).to.be.an.instanceOf(Error);
        expect(error.message).to.include(
          "Unable to determine algorithm based on provided input"
        );
      }
    });
  });
